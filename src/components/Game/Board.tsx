...
const Board = () => {
  ...
  useEffect(() => {
    ...
    if(shuffledTiles.length > 0 && !shuffledTiles.find((element)=>element.isMatched===false)){
...
if (!statistics.find(el => el.id === attempt.id)) {
  ...
  return (
    <div className={`${styles.board} ${styles[difficulty]}`}>
      {endGame ? <ModalWindow > <h3>Game won</h3>
            <Stats/>
            <Button style='primary' onClick={resetGame} >Play again</Button>
            <p className={styles.small} onClick={()=>setIsStatsOpen(true)}><u>show all attempts</u></p></ModalWindow> : (
